...
public sealed class RestoreProcessorTests
{
    ...
    public void DiscoverBackups_WhenBackupsDirectoryDoesNotExist_ReturnsEmptyCollection()
    {
        ...
        var backups = _processor.DiscoverBackups(instance);

        Assert.IsEmpty(backups);
    }

    /// <summary>
    /// Verifies that incomplete and non-backup directories are not returned as completed backups.
    /// </summary>
    [TestMethod]
    public void DiscoverBackups_WhenDirectoriesHaveNoManifest_SkipsDirectories()
    {
        Directory.CreateDirectory(_backupsPath);
        Directory.CreateDirectory(Path.Combine(_backupsPath, ".in-progress-test"));
        Directory.CreateDirectory(Path.Combine(_backupsPath, "Not A Backup"));
    ...
    /// <summary>
    /// Verifies that malformed manifest JSON prevents a backup from being treated as valid.
    /// </summary>
    [TestMethod]
    public void DiscoverBackups_WhenManifestContainsInvalidJson_ThrowsInvalidDataException()
    {
        ...
        File.WriteAllText(Path.Combine(backupPath, "manifest.json"), "{ invalid JSON }");
        ...
        Assert.ThrowsExactly<InvalidDataException>(
            () => _processor.DiscoverBackups(instance)
        );
    }
